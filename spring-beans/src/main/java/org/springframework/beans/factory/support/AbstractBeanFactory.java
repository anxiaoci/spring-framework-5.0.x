...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	 ...
	 * Spring容器维护了一个BeanPostProcessor的list集合，可以插手Bean的实例化过程
	 * 在Bean的实例化过程中，会循环这个list，依次 执行插手Bean的实例化过程
	 */
	 ...
	 * BeanDefinition合并以后都存放在mergedBeanDefinitions中
	 */
	...
	protected <T> T doGetBean(final String name, @Nullable final Class<T> requiredType,
		...
		if (sharedInstance != null && args == null) {
			...
			try {
				...
				// Create bean instance.，判断是否是单例对象
				if (mbd.isSingleton()) {
					//在这次getSingleton方法中，创建bean时 加入到正在创建map中
	...
	public void addBeanPostProcessor(BeanPostProcessor beanPostProcessor) {
		...
		//添加BeanPostProcessor，每次都是先删除后添加
	...
	public BeanDefinition getMergedBeanDefinition(String name) throws BeansException {
		...
		//未已存入beanDefinitionMap && 父Bean工厂为ConfigurableBeanFactory
		if (!containsBeanDefinition( beanName ) && getParentBeanFactory() instanceof ConfigurableBeanFactory) {
			return ((ConfigurableBeanFactory) getParentBeanFactory()).getMergedBeanDefinition( beanName );
		}
		// Resolve merged bean definition locally.
		//已已存入beanDefinitionMap || 父Bean工厂不为ConfigurableBeanFactory
	...
	/**
	 * 使用普通编辑器(已经使用BeanFactory注册)初始化给定的属性编辑器注册器
	...
	/**
	 * 合并一个给定Bean的RootBeanDefinition，合并内容包括 父BeanDefinition
	 ...
	 *                     or {@code null} in case of a top-level bean(内部bean活着顶级bean)
	 * @return a (potentially merged) RootBeanDefinition for the given bean
	 * @throws BeanDefinitionStoreException in case of an invalid bean definition
	 */
		...
		synchronized (this.mergedBeanDefinitions) {
			...
			if (mbd == null) {
				...
				if (bd.getParentName() == null) {
					...
					//没有父类bean，就复制一份当前的bean的BeanDefinition用于后续处理
					...
					//如果有父类BeanDefinition：需要与父类BeanDefinition合并
					BeanDefinition pbd;
					try {
						//获取父bean的原生名称
						String parentBeanName = transformedBeanName( bd.getParentName() );
						//当前bean的名称与父类bean名称不是同一个，则合并
				...
				//把合并后的bean存入mergedBeanDefinitions(已合并BeanDefinition)中
				//可能会由于元数据的改变，在后续过程中再合并
