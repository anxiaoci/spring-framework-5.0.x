...
class ConstructorResolver {
	...
	public BeanWrapper autowireConstructor(String beanName, RootBeanDefinition mbd,
		...
		//实例化对象使用的构造器参数（在使用@Component方式，没有告诉他这个属性）
		...
		if (explicitArgs != null) {
			...
			synchronized (mbd.constructorArgumentLock) {
				//获取已解析的构造方法，一般不会有，因为构造方法一般会提供一个。除非提供多个构造方法才会存在已经解析完成的构造方法
		...
		if (constructorToUse == null) {
			...
			/**
			 * 定义了最小参数个数
			 * 如果给的构造方法的参数列表给定了具体的值，那么这些值的个数就是构造方法参数的个数
			 */
			int minNrOfArgs;
			//explicitArgs是从调用者传入的构造方法的值
			if (explicitArgs != null) {
				minNrOfArgs = explicitArgs.length;
			} else {
				//指定参数为空，实例化一个对象，用来存放构造方法的参数，当中主要存放了参数值和参数值所对应的下标
				ConstructorArgumentValues cargs = mbd.getConstructorArgumentValues();
				resolvedValues = new ConstructorArgumentValues();
				/**
				 * 确定构造方法的参数数量，xml中配置
				 * <bean>
				 *     <contructor-arg index="0" value=""></>
				 *     <contructor-arg index="1" value=""></>
				 * </bean>
				 * minNrOfArgs=2
				 */
		...
		try {
			/**
			 * 使用反射创建实例  look-method 通过cglib增强bean实例
			 */
